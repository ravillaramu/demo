...
$.fn.appendItem = function (options) {
  ...
  var elementHtml =
    ...
    if(settings.allowCrossfilter) {
      elementHtml = elementHtml + " <img src='images/crossfilter1.png' title='This Icon denotes cross-filter can be applied to this chart' alt='crossfilterIcon'  class='crossfilter' />"
    }

    elementHtml = elementHtml + "</div>";
  if (settings.widgetType == "composite-stacked-bar-chart") {
    ...
    if (settings.dataFromIndividualAPI) {
      ...
      let dataToFilter = [];
      if (settings.responseData[settings.datafield]) {
        let keyArr = Object.keys(settings.responseData[settings.datafield]);
    ...
    if (settings.dataFromIndividualAPI) {
      ...
      let sortdataToFilter = dataToFilter.sort((a, b) => {//NOSONAR
      ...
      if (settings.labelSpecificColors && settings.labelSpecificColors.length > 0) {
        ...
        colors = chartColors.reverse();//NOSONAR
        settings.colors = chartColors;
        dataToFilter = dataToFilter.reverse();//NOSONAR
      }
      drawRadiusPieChart(settings, dataToFilter);
    }
    ...
    if (settings.dataFromIndividualAPI) {
      ...
      if (dataToFilter && dataToFilter.length > 0) {
        ...
        var noRecordFoundDiv = document.createElement("div");
        noRecordFoundDiv.textContent = "NO DATA FOUND FOR THE CURRENT FILTER";
        noRecordFoundDiv.classList.add("no-result");
        $(settings.target).append(noRecordFoundDiv);
      }
    }
    ...
    if (settings.withFilter) {
      ...
      for (var i = 0; i < filterFields.length; i++) {
        if (radioBtns == "") {
          radioBtns = '<input type="radio" id="' + filterFields[i] + '" value="' + filterFields[i] + '" name="' + settings.id + 'RadioFilter"></input><label for="' + filterFields[i] + '" class="sr-only">' + filterFields[i] + '</label> <br>';
        } else {
          radioBtns = radioBtns + '<input type="radio" id="' + filterFields[i] + '" value="' + filterFields[i] + '" name="' + settings.id + 'RadioFilter"></input><label for="' + filterFields[i] + '" class="sr-only">' + filterFields[i] + '</label> <br>';
        }
      }
        ...
        allRadioBtns[i].addEventListener("change", function () {
          ...
          if (settings.dataFromIndividualAPI) {
            ...
            if (DataToProcess && DataToProcess.length > 0) {
              ...
              var noRecordFoundDiv = document.createElement("div");
              noRecordFoundDiv.textContent = "NO RECORDS FOUND FOR THE APPLIED FILTER";
              $(settings.target).append(noRecordFoundDiv);
            }
          }
        });
    ...
    if (settings.dataFromIndividualAPI) {
      ...
      var newcolorArr = settings.colors;
      var mockData = [{
        name: "IND",
        data: [{
          name: "IND",
      ...
      if (DataToProcess && DataToProcess.length > 0) {
        ...
        noDataFoundDiv.textContent = "NO DATA FOUND FOR THE CURRENT FILTER";
        noDataFoundDiv.classList.add("no-result");
        $(settings.target).append(noDataFoundDiv);
      }
    }
    ...
    if (settings.withFilter) {
      ...
      // filterDiv.style("display", "flex")
      filterDiv.classList.add("flex");
      let filterFields = settings.FilterFields;
      let radioBtns = "";
      for (var radioCount = 0; radioCount < filterFields.length; radioCount++) {
        if (radioBtns == "") {
          radioBtns = '<input type="radio" id="' + filterFields[i] + '" value="' + filterFields[i] + '" name="' + settings.target + 'RadioFilter"></input><label for="' + filterFields[i] + '" class="sr-only">' + filterFields[i] + '</label> <br>';
        } else {
          radioBtns = radioBtns + '<input type="radio" id="' + filterFields[i] + '" value="' + filterFields[i] + '" name="' + settings.target + 'RadioFilter"></input><label for="' + filterFields[i] + '" class="sr-only">' + filterFields[i] + '</label> <br>';
        }
      }
      filterDiv.innerHTML = radioBtns;
      $(settings.target).append(filterDiv);
...
function drawD3RadarChart(dataToFilter, settings) {
  ...
  dataToFilter.forEach((res, index, arr) => {
    if (CalculatedData.some(item => {
      if (item.key === res[settings.dimension]) {//NOSONAR
        return true;
      } else {
        return false;
      }
  ...
  var colors = settings.colors;
  var data = CalculatedData.sort((a, b) => b.value - a.value).map((dataval, i) => {
    return {
...
function drawPieChart(settings, seriesData) {
  ...
  if(settings.allowCrossfilter) {
    title = "<span class='charttitle' style=' display: block; cursor: pointer' title='This Icon denotes cross-filter can be applied to this chart'>"+ title + "<img alt='crossfilterIcon'  class='crossfilter' /></span>";
  }
  Highcharts.chart(settings.id, {
    chart: {
    ...
    title: {
      ...
      useHTML: settings.allowCrossfilter
    },
    legend: {
      layout: settings.legendLayout,
      align: settings.legendAlign,
